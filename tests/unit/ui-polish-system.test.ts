...
describe("shared UI polish system", () => {
  ...
  it("polishes the named forum and privacy surfaces with shared patterns", () => {
    ...
    expect(thread).toContain("ForumDeleteThreadControl");
    expect(thread).toContain("group w-full min-w-0");
    ...
    expect(deleteControl).toContain("deleteThreadAction");
    expect(deleteControl).toContain("Delete this post?");
    expect(deleteControl).toContain("Delete this thread?");
    const category = read("app/(member)/app/forum/[slug]/page.tsx");
    expect(category).toContain("ForumDeleteThreadControl");
    const list = read("lib/forum/list.ts");
    expect(list).toContain("!post.deletedAt");
    expect(list).toContain("authorId: row.authorId");
